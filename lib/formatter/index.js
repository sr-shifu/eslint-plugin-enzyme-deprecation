...
module.exports = (results, options) => {
  ...
  visualizer(processResults(files, options.valueFormatterType));

  log("Rules", "rectangles");
  visualizer(processResults(rules, options.valueFormatterType));

  log("Summary", "rectangles");
  visualizer(processResults(summary, `${options.valueFormatterType}-summary`));
};
